...
class MimisbrunnVault:
    """Encrypted append-only Merkle vault (Mímisbrunn) - Immutable DB for Credo/BrQin."""
    ...
    def load_state(self, step: int = -1):
        if step == -1:
            ...
            if not checkpoints:
                raise FileNotFoundError("No checkpoints found in vault")
    ...
    def verify_chain(self) -> Tuple[bool, str]:
        for i, entry in enumerate(self.merkle_chain):
            ...
            computed_hash = self._compute_hash(without_root)
            if entry.get("root_hash") != computed_hash:
                return False, f"Tamper detected at entry {i} (event_type: {entry.get('event_type')})"
        return True, f"Chain intact ({len(self.merkle_chain)} events verified)"

    def get_event_history(self, event_type: Optional[str] = None) -> List[Dict]:
        if event_type is None:
            return self.merkle_chain
        return [e for e in self.merkle_chain if e["event_type"] == event_type]

if __name__ == "__main__":
    vault = MimisbrunnVault()
    print("✅ MimisbrunnVault (encrypted append-only Merkle DB) initialized successfully.")
    print(f"Vault directory: {VAULT_DIR}")
    print(f"Encryption enabled: {ENCRYPTION_AVAILABLE}")